...
eventRoutes.get("/data/:title", async (req, res) => {
  try {
    ...
    const [results] = await db.execute("SELECT * FROM event WHERE title = ?", [
      title,
    ]);
...
eventRoutes.get("/eventpage/:category", async (req, res) => {
  try {
    ...
    const [results] = await db.execute(
      "SELECT * FROM event WHERE category = ?",
      [category]
    );
...
eventRoutes.post("/upload", singleUpload, async (req, res) => {
  try {
    ...
    const [results] = await db.execute(
      "INSERT INTO event (title, image_url, content, location, category, event_date) VALUES (?, ?, ?, ?, ?, ?)",
      [title, image_url.filename, content, location, category, event_date]
    );
    res.json(results);
  } catch (err) {
    console.error("Error fetching data from reports:", err);
...
eventRoutes.put("/update/:id", singleUpload, async (req, res) => {
  try {
    ...
    const { title, content, location, category, event_date } = req.body;
    const [results] = await db.execute(
      "UPDATE event SET title = ?, image_url = ?, content = ?, location = ?, category = ?, event_date = ? WHERE id = ?",
      [title, image_url.filename, content, location, category, event_date, id]
    );
    res.json(results);
  } catch (err) {
    console.error("Error fetching data from reports:", err);
