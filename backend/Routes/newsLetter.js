...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, "public/newsletterUpload");
  },
...
newsRoutes.get("/data/:title", async (req, res) => {
  try {
    ...
    const [rows] = await db.execute(
      "SELECT * FROM newsletter WHERE title = ?",
      [title]
    );
...
newsRoutes.post("/upload", multipleUploads, async (req, res) => {
  try {
    ...
    const [results] = await db.execute(
      "INSERT INTO newsletter (title, image_url, created_date, modified_date, created_by, modified_by, pdf_url) VALUES (?, ?, ?, ?, ?, ?, ?)",
      [
        payload.title,
...
newsRoutes.delete("/delete/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const [rows] = await db.execute(
      "SELECT pdf_url FROM newsletter WHERE id = ?",
      [id]
    );
